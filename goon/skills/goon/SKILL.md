...
GoOn 把"上次会话做了什么"蒸馏成一份随 git 版本化的结构化交接（handoff），换到别的 agent 客户端后，用它 + 一份 git 差异报告（Drift Report）让新 agent 直接接手。**GoOn 只读各客户端的会话文件，绝不写入；交接写进项目的 `.goon/` 目录。**
...
保存交接（换端前，或察觉自己将不可用时）：
- `goon distill <source> <session-file>` — 主路径：读取会话文件→裁剪→调 LLM 蒸馏→写 `.goon/handoffs/<id>.md`。`<source>` ∈ `claude-code` `codex`（P1 支持这两个；opencode/zcode 的 SQLite 后端在 P2）。
- `goon salvage <source> <session-file>` — 兜底：LLM 端点也不可用时，**不调 LLM**，纯代码把会话抽成 `.goon/salvage/*.raw.md` 草稿（供新 agent 自行消化）。
...
## 各端会话文件位置（供 distill/salvage 指定 `<session-file>`）
...
- opencode / zcode：SQLite 存储（`~/.local/share/opencode/opencode.db`、`~/.zcode/cli/db/db.sqlite`），P1 暂不支持，P2 加入。
